...
//Taken from org.matheusdev.util.BayazitDecomposer.java
//https://github.com/matheus23/RuinsOfRevenge/blob/master/src/org/matheusdev/util/BayazitDecomposer.java
...
public class BayazitDecomposer {
	...
	public static Vector2 cross(final Vector2 a, final float s) {
	...
	public static float getSignedArea(final Vector2[] vect) {
		int i;
		float area = 0;
	...
	public static Boolean isCounterClockWise(final Vector2[] vect) {
		// We just return true for lines
		if (vect.length < 3) return true;
		return (getSignedArea(vect) > 0.0f);
	}
	...
	public static List<List<Vector2>> convexPartition(final List<Vector2> vertices) {
		...
		if (!isCounterClockWise(vertices)) {
			...
			Collections.reverse(vertices);
			// ArrayList<Vector2> reversed = new ArrayList<Vector2>(vertices.size());
		...
		ArrayList<Vector2> lowerPoly, upperPoly;
	...
	public static Vector2 lineIntersect(final Vector2 p1, final Vector2 p2, final Vector2 q1, final Vector2 q2) {
	...
	public static Boolean floatEquals(final float value1, final float value2) {
	...
	// otherwise.</returns>
	public static Boolean lineIntersect(final Vector2 point1, final Vector2 point2, final Vector2 point3,
			final Vector2 point4, final Boolean firstIsSegment, final Boolean secondIsSegment, Vector2 point) {
		point = new Vector2();
	...
	private static Boolean left(final Vector2 a, final Vector2 b, final Vector2 c) {
		return area(a, b, c) > 0;
	}

	private static Boolean leftOn(final Vector2 a, final Vector2 b, final Vector2 c) {
		return area(a, b, c) >= 0;
	}

	private static Boolean right(final Vector2 a, final Vector2 b, final Vector2 c) {
		return area(a, b, c) < 0;
	}

	private static Boolean rightOn(final Vector2 a, final Vector2 b, final Vector2 c) {
		return area(a, b, c) <= 0;
	}

	public static float area(final Vector2 a, final Vector2 b, final Vector2 c) {
		return a.x * (b.y - c.y) + b.x * (c.y - a.y) + c.x * (a.y - b.y);
	}

	private static float squareDist(final Vector2 a, final Vector2 b) {
		float dx = b.x - a.x;
		float dy = b.y - a.y;
		return dx * dx + dy * dy;
	}
...
class SimplifyTools {
	...
	public static List<Vector2> collinearSimplify(final List<Vector2> vertices,
		...
		for (int i = 0; i < vertices.size(); i++) {
			int prevId = i - 1;
			if (prevId < 0) prevId = vertices.size() - 1;
			int nextId = i + 1;
			if (nextId >= vertices.size()) nextId = 0;
			Vector2 prev = vertices.get(prevId);
			Vector2 current = vertices.get(i);
			Vector2 next = vertices.get(nextId);
			// If they collinear, continue
			if (collinear(prev, current, next, collinearityTolerance)) continue;
			simplified.add(current);
		}
		return simplified;
	}

	public static Boolean collinear(final Vector2 a, final Vector2 b, final Vector2 c, final float tolerance) {
		return floatInRange(BayazitDecomposer.area(a, b, c), -tolerance, tolerance);
	}

	public static Boolean floatInRange(final float value, final float min, final float max) {
	...
	public static ArrayList<Vector2> douglasPeuckerSimplify(final ArrayList<Vector2> vertices,
			final float distanceTolerance) {
		...
		for (int i = 0; i < vertices.size(); i++)
			if (_usePt[i]) result.add(vertices.get(i));
		return result;
	}

	private static void simplifySection(final ArrayList<Vector2> vertices, final int i, final int j) {
		if ((i + 1) == j) return;
	...
	private static double distancePointPoint(final Vector2 p, final Vector2 p2) {
		double dx = p.x - p2.x;
		double dy = p.y - p2.x;
		return Math.sqrt(dx * dx + dy * dy);
	}

	private static double distancePointLine(final Vector2 p, final Vector2 A, final Vector2 B) {
	...
	// From physics2d.net
	public static ArrayList<Vector2> reduceByArea(final ArrayList<Vector2> vertices, float areaTolerance) {
		if (vertices.size() <= 3) return vertices;
		if (areaTolerance < 0) {
	...
	public static void mergeParallelEdges(final ArrayList<Vector2> vertices, final float tolerance) {
		...
		ArrayList<Vector2> oldVertices = new ArrayList<Vector2>(vertices);
		vertices.clear();
	...
	// / <param name="vertices">The vertices.</param>
	// / <returns></returns>
	public static ArrayList<Vector2> mergeIdenticalPoints(final ArrayList<Vector2> vertices) {
	...
	// / <returns></returns>
	public static ArrayList<Vector2> reduceByDistance(final ArrayList<Vector2> vertices, final float distance) {
		// We can't simplify polygons under 3 vertices
		if (vertices.size() < 3) return vertices;
		ArrayList<Vector2> simplified = new ArrayList<Vector2>();
		for (int i = 0; i < vertices.size(); i++) {
	...
	// / <returns></returns>
	public static ArrayList<Vector2> reduceByNth(final ArrayList<Vector2> vertices, final int nth) {
		// We can't simplify polygons under 3 vertices
		if (vertices.size() < 3) return vertices;
		if (nth == 0) return vertices;
		ArrayList<Vector2> result = new ArrayList<Vector2>(vertices.size());
		for (int i = 0; i < vertices.size(); i++) {
			if (i % nth == 0) continue;
